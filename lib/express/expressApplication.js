...
function initializeApplication(expressApp) {
  ...
  // Errors/NotFound
  if (!expressApp.errorNotFoundHandler) {
    expressApp.useErrorNotFoundHandler(defaultErrorNotFoundHandler());
  }

  if (expressApp.errorHandler) {
    expressApp.useErrorHandler(defaultErrorHandler(expressApp));
  }
