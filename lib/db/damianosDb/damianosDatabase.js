...
const encryptionService = new EncryptionService({
  encryptionKey: process.env.ENCRYPTION_KEY_DAMIANOS_IO,
});
...
class DamianosDatabase extends MongoDatabase {
  constructor(connection, name, logger) {
  ...
  static async createDatabase({
    ...
    try {
      ...
      const database = new DamianosDatabase(connection, name, logger);
  ...
  async createApplicationAuthorizationCode(createApiClientApplicationAuthorizationCodeParameters) {
    ...
    try {
      ...
      let code;
      if (appIndex >= 0) {
        apiClient.applications[appIndex] = await ApiClientApplication.generateAuthorizationCode(apiClient.applications[appIndex]);
        const updateOptions = {
          applications: apiClient.applications,
          modifiedAt: this.now(),
        };
  ...
  async authorizeApplication(authorizeApplicationParameters) {
    ...
    try {
      ...
      if (appIndex >= 0) {
        ...
        const { accessToken, refreshToken, accessTokenExpiresIn /*, refreshTokenExpiresIn*/ } = await application.createTokens();
