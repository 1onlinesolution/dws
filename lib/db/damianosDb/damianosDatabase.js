const {
  ...
  Converter,
  DateTimeUtils,
} = require('@1onlinesolution/dws-utils');
const {
  ObjectId,
  MongoDatabase,
  MongoCollection,
...
class DamianosDatabase extends MongoDatabase {
  constructor({
    ...
    if (!Validity.isValidString(application_name, 2)) throw new Error('invalid application name');
  ...
  async findUserByUserName(user_name) {
    if (!user_name) return Promise.reject(new Error('invalid user name'));
    try {
      const document = await this.users.findOne({ user_name: user_name });
      if (document) return new User(document);
      return null;
    } catch (err) {
      return Promise.reject(err);
  ...
  //             application_names.push(app.application_name);
  //           });
  //         });
  //       } /*, err => {
  //         if (err) ...
  ...
  async createUser(createUserParameters) {
    ...
    try {
      ...
      user = new User({
        company_name: createUserParameters.company_name,
        first_name: createUserParameters.first_name,
        last_name: createUserParameters.last_name,
        user_name: createUserParameters.user_name,
        email: createUserParameters.email,
        auto_verify: createUserParameters.auto_verify,
  ...
  async loginVerifiedUser(loginUserParameters) {
    ...
    try {
      ...
      if (!user) return Promise.reject(new Error('invalid email/user_name and/or password'));
      ...
      const { access_token, refresh_token } = await DamianosDatabase.createVerifiedUserTokens(user);
      user.jwt_access_token = access_token;
      ...
      await this.updateUserCore(User.id(user), {
        ...
        refresh_token_created_at: nowUtc,
        modified_at: nowUtc,
      });
      delete user.password;
  ...
  async updateUser(updateUserParameters) {
    ...
    if (error) return Promise.reject(error);
    const { user_id, first_name, last_name, newsletter } = updateUserParameters;
    try {
  ...
  async createUserAddress(createUserAddressParameters) {
    ...
    try {
      ...
      if (!addresses || addresses.length === 0) {
        ...
        if (addressWithDefaultId) {
          await this.updateAddressCore(addressWithDefaultId, { isDefault: false, modified_at: this.now() });
        }

        if (addressWithBillingId) {
          await this.updateAddressCore(addressWithBillingId, { isBilling: false, modified_at: this.now() });
        }

        if (addressWithShippingId) {
          await this.updateAddressCore(addressWithShippingId, { isShipping: false, modified_at: this.now() });
        }
      }
  ...
  async deleteUserAddress(deleteUserAddressParameters) {
    ...
    try {
      ...
      if (addresses && addresses.length > 0) {
        ...
        if (hasDefaultId) {
          await this.updateAddressCore(addresses[0]._id, { isDefault: true, modified_at: this.now() });
        }
        if (hasBillingId) {
          await this.updateAddressCore(addresses[0]._id, { isBilling: true, modified_at: this.now() });
        }
        if (hasShippingId) {
          await this.updateAddressCore(addresses[0]._id, { isShipping: true, modified_at: this.now() });
        }
      }
  ...
  async findApplicationByAppClientId(api_client_id) {
    if (!api_client_id) return Promise.reject(new Error('invalid API client identifier'));
    return await this.apiClientApplications.find({ api_client_id: ObjectId(api_client_id) });
  }

  async findApiClientApplicationById(api_client_id, api_client_application_id) {
    if (!api_client_id) return Promise.reject(new Error('invalid api client identifier'));
    if (!api_client_application_id) return Promise.reject(new Error('invalid api client application identifier'));

    try {
      const document = await this.apiClientApplications.findOne({ _id: ObjectId(api_client_application_id), api_client_id: api_client_id });
  ...
  async findApiClientApplication(api_client_id, application_name, website_url) {
    if (!api_client_id) return Promise.reject(new Error('invalid API client identifier'));
    if (!application_name) return Promise.reject(new Error('invalid API client application name'));
    if (!website_url) return Promise.reject(new Error('invalid API client application website URL'));

    try {
      const document = await this.apiClientApplications.findOne({ api_client_id, application_name, website_url });
      if (document) return new ApiClientApplication(document);
      return null;
    } catch (err) {
      return Promise.reject(err);
  ...
  async findApiClientApplicationByAuthorizationCode(authorization_code) {
    if (!authorization_code) return Promise.reject(new Error('invalid API client application authorization code'));

    try {
      const document = await this.apiClientApplications.findOne({ authorization_code });
      if (document) return new ApiClientApplication(document);
      return null;
    } catch (err) {
      return Promise.reject(err);
  ...
  async findApiClientApplicationByRefreshToken(refresh_token) {
    if (!refresh_token) return Promise.reject(new Error('invalid API client application refresh token'));

    try {
      const document = await this.apiClientApplications.findOne({ refresh_token });
      if (document) return new ApiClientApplication(document);
      return null;
    } catch (err) {
      return Promise.reject(err);
  ...
  async registerApiClientApplication(createApiClientApplicationParameters) {
    ...
    try {
      const { api_client_id, application_name, website_url } = createApiClientApplicationParameters;
      const apiClientApp = await this.findApiClientApplication(api_client_id, application_name, website_url);
  ...
  async updateApiClientApplication(updateApiClientApplicationParameters) {
    ...
    try {
      let apiClientApplication = await this.findApiClientApplicationById(api_client_application_id);
  ...
  async generateApplicationAuthorizationCode(createApiClientApplicationAuthorizationCodeParameters) {
    ...
    if (error) return Promise.reject(error);
    const { api_client_id, api_client_application_id /*, redirect_url*/ } = createApiClientApplicationAuthorizationCodeParameters;

    try {
      const apiClientApp = await this.findApiClientApplicationById(api_client_id, api_client_application_id);
      ...
      const updateOptions = {
        authorization_code: modifiedApiClientApp.authorization_code,
      ...
      if (await this.updateApiClientApplicationCore(apiClientApp, updateOptions)) {
        return modifiedApiClientApp.authorization_code;
      }

      return await this.updateApiClientApplicationCore(apiClientApp, updateOptions)
        ? modifiedApiClientApp.authorization_code
        : null;
    } catch (err) {
      return Promise.reject(err);
    }
  ...
  async authorizeApplication(authorizeApiClientApplicationParameters) {
    ...
    if (error) return Promise.reject(error);
    const { authorization_code, api_client_id, api_client_secret } = authorizeApiClientApplicationParameters;

    try {
      const apiClientApp = await this.findApiClientApplicationByAuthorizationCode(authorization_code);
      if (!apiClientApp) return Promise.reject(new Error('API client application not found'));

      // We found the client application
      const result = await apiClientApp.createTokens();
      const { access_token, refresh_token, access_token_expires_in, refresh_token_expires_in } = result;
      const updateOptions = {
        access_token,
        refresh_token,
  ...
  async refreshApplicationToken(refreshApplicationAccessTokenParameters) {
    ...
    const { refresh_token, api_client_id, api_client_secret } = refreshApplicationAccessTokenParameters;

    try {
      const user = await this.findUserByApiClientId(api_client_id);
      if (!user) return Promise.reject(new Error('API client identifier not found'));

      const apiClientApp = await this.findApiClientApplicationByRefreshToken(refresh_token);
      if (!apiClientApp) return Promise.reject(new Error('API client application not found'));
      if (apiClientApp.api_client_id !== api_client_id) return Promise.reject(new Error('invalid API client identifier'));
      if (user.api_client_secret !== api_client_secret) return Promise.reject(new Error('invalid API client secret'));
  ...
  User ${user.first_name} (email: ${user.email}, username: ${user.user_name}) has registered.

  Please take action. 
  
  ${DamianosDatabase.emailSignature(ip, application_name)}
`,
    };
  }
...
`,
  ...
  static createPasswordResetConfirmationEmailMessage(emailUserDataParameters) {
    ...
    return {
      from: 'team@damianos.io',
      to: user.email,
      subject: `${application_name} account - Password reset confirmation`,
      text: `
  Hi ${user.first_name},
...
`,
  ...
  static createLoginAttemptEmailMessage(emailUserDataParameters) {
    ...
    return {
      from: 'team@damianos.io',
      to: user.email,
      subject: `Security notification regarding your ${application_name} account`,
      text: `
  Hi ${user.first_name},
...
`,
  ...
  static emailSignature(ip, application_name) {
    return `Detected IP address: ${ip}

  Application: ${application_name}
  
  Your team @ ${application_name}`;
  }
}
