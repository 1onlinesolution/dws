...
class ExpressApplication {
  constructor({
    ...
    if (useLog) {
      ...
      if (logUseMongoDB) {
        if (!logUseMongoDB) throw new Error('invalid log MongoDB url');
    ...
    if (this.useLog) {
      this.app.use(morgan(this.logOptions.morganFormat, { stream: this.logger.stream }));
    }
    ...
    if (!this.errorHandler) this.errorHandler = defaultErrorHandler(this.isApi);
  ...
  get logOptions() {
    if (!this._logOptions) {
      this._logOptions = {
        ...
        mongoLevel: this.logMongoLevel,
        consoleLevel: this.logConsoleLevel,
        mongoUri: this.logMongoUrl,
        mongoCollectionName: this.logMongoCollectionName,
        morganFormat: this.logFormat,
      };
  ...
  logWarn(message) {
    if (this.useLog) this.logger.warn(message);
  }

  logInfo(message) {
    if (this.useLog) this.logger.info(message);
  }

  logError(message) {
    if (this.useLog) this.logger.error(message);
  }
...
function defaultErrorHandler(/*isApi*/) {
