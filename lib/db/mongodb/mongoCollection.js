...
class MongoCollection {
  constructor(database, collectionName) {
    if (!database || !Validity.isValidString(database.name)) {
      throw new Error('invalid database');
    }

    if (!Validity.isValidString(collectionName)) {
  ...
  get collection() {
    return this.database.database.collection(this._collectionName);
  }
  ...
  async createIndexes(indexMap) {
    ...
    try {
      await this.ensureCollectionExists(this._collectionName);
      let promises = [];
  ...
  //   return await this.collection.indexExists(indexName);
  // }

  // async createIndexCore(fieldOrSpec, options) {
  //   return await this.collection.createIndex(fieldOrSpec, options);
  // }

  async ensureCollectionExists(collectionName) {
    if (!(await this.database.collectionExists(collectionName))) {
