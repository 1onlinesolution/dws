...
const AddUserAddressParameters = require('../../models/documents/sso/requestParams/addUserAddressParameters');
...
class DamianosDatabase extends MongoDatabase {
  constructor({
    ...
    this._countries = new MongoCollection(this, 'countries');
    this._statesUS = new MongoCollection(this, 'statesUS');
  ...
  async addUserAddress(addUserAddressParameters) {
    const error = AddUserAddressParameters.checkForError(addUserAddressParameters);
    if (error) return Promise.reject(error);
    const { user_id } = addUserAddressParameters;
    ...
    try {
      ...
      if (User.findAddressIndex(user, addUserAddressParameters.address) >= 0) return Promise.reject(new Error('address already exists'));

      if (user.addresses.length === 0) {
        // If this is the first address, this will be the default address
        addUserAddressParameters.address.isDefault = true;
        addUserAddressParameters.address.isBilling = true;
        addUserAddressParameters.address.isShipping = true;
      } else {
        if (addUserAddressParameters.address.isDefault) {
        ...
        if (addUserAddressParameters.address.isBilling) {
          user.addresses.forEach(address => {
        ...
        if (addUserAddressParameters.address.isShipping) {
          user.addresses.forEach(address => {
      ...
      user.addresses.push(addUserAddressParameters.address);
      return await this.updateUserCore(user_id, {
  ...
  async updateUserAddress(updateUserAddressParameters) {
    ...
    if (error) return Promise.reject(error);
    const { user_id } = updateUserAddressParameters;
    ...
    try {
      ...
      const index = User.findAddressIndex(user, updateUserAddressParameters.address);
      if (index < 0) return Promise.reject(new Error('address does not exist'));

      // eslint-disable-next-line security/detect-object-injection
      if (updateUserAddressParameters.address.isDefault && user.addresses[index].isDefault === false) {
        // Another address is default; mark all as non default
        user.addresses.forEach(address => {
          if (address.isDefault) {
            address.isDefault = false;
          }
      ...
      // eslint-disable-next-line security/detect-object-injection
      if (updateUserAddressParameters.address.isBilling && user.addresses[index].isBilling === false) {
        // Another address is billing; mark all as non billing
        user.addresses.forEach(address => {
          if (address.isBilling) {
            address.isBilling = false;
          }
      ...
      // eslint-disable-next-line security/detect-object-injection
      if (updateUserAddressParameters.address.isShipping && user.addresses[index].isShipping === false) {
        // Another address is shipping; mark all as non shipping
        user.addresses.forEach(address => {
          if (address.isShipping) {
            address.isShipping = false;
          }
      ...
      // eslint-disable-next-line security/detect-object-injection
      user.addresses[index] = updateUserAddressParameters.address;
      return await this.updateUserCore(user_id, {
        addresses: user.addresses,
        modifiedAt: this.now(),
      });
  ...
  async deleteUserAddress(deleteUserAddressParameters) {
    ...
    if (error) return Promise.reject(error);
    const { user_id } = deleteUserAddressParameters;
    ...
    try {
      ...
      const index = User.findAddressIndex(user, deleteUserAddressParameters.address);
      if (index < 0) return Promise.reject(new Error('cannot find address'));

      // We do not want to delete a default address
      // eslint-disable-next-line security/detect-object-injection
      if (user.addresses[index].isDefault) return Promise.reject(new Error('cannot delete the default address'));

      return await this.updateUserCore(user_id, {
        addresses: user.addresses.splice(index, 1),
        modifiedAt: this.now(),
      });
    } catch (err) {
      return Promise.reject(err);
